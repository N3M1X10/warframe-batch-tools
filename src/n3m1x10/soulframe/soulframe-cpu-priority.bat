...
:set-priority
powershell -Command "do {Start-Sleep -Seconds 2; $proc = Get-Process -Name "Soulframe.x64" -ErrorAction SilentlyContinue; $hasWindow = $false; if ($proc -and $proc.MainWindowHandle -ne 0) { $hasWindow = $true; Write-Host "Process has been found"; $proc.PriorityClass = [System.Diagnostics.ProcessPriorityClass]::!priority!; Write-Host "Priority for Soulframe.x64.exe has been changed"} else{cls; Write-Host "Process Soulframe.x64.exe has no window"; Write-Host "Im trying to find him again . . ."}} while (-not $hasWindow)"
