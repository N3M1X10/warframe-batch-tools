...
2. Генерируем свой конфиг WARP.
  - Заходим на [**этот сайт**](https://generator-warp.vercel.app/) и генерируем конфиг для AmneziaWG (выберите свой тип подключения к сети: проводной, мобильный итд). Скачается файл с расширением `.conf`.
  - Либо на [**этот репозиторий**](https://github.com/ImMALWARE/bash-warp-generator) и ищем там информацию для генерации.
3. В окне AmneziaWG внизу слева, нажимаем **`Добавить туннель`** и ищем скачанный конфиг файл (либо создаём свой файл, когда генерируем в консоли)
...
## 📝 Примечания
>[!note]
>- Проверено лично, на работоспособность: `09.05.2025`
>- Гайд обновлён: `10.01.2026`

- Автор гайда не разбирался с обходом на `ipv6`. Просьба сильно не ругаться.
...
- При использовании AmneziaWG - по умолчанию, требуется отключить любые VPN или прокси. Либо настройте маршрутизацию того IP чата в своём VPN
